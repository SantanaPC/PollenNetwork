...
## Visualization of a pollen network
...
pool_net <- read.csv("1.Data/Network_pooled_data.csv", header = T, sep="," )

plantInt<-net_info|> filter(resource==1)|> 
  filter(total_interactions>=1) 
...
pollen_bi<-ifelse(pollen_net>0, 1, 0)
...
m<-glm(pollen~1, family= poisson, data=bee)
bbmle::AICtab(m, m1, m2, weights=TRUE)

ggplot(bee, aes(x=bee_genus, y=pollen)) +  geom_violin(aes(fill=bee_genus), alpha=0.2)+ geom_boxplot(width=0.1, alpha=0.3) +
scale_fill_manual(values=c("#714925", "#CF6B13","#F69F22"))+geom_point(aes(color=hive), size=3) +  scale_color_viridis(discrete = TRUE)+
labs(x="Bee species", y="Number of interactions with plant species")+theme_bw()

```

```{r checking interaction with native and non-native species}

consStatus<-left_join(net_info[,c(2,6,8)], pool_net, by = "species")
...
scale_color_manual(values=c("#d7191c","#eec04b")) +facet_wrap(~BeeSpecies)+
labs(x="Conservation Status of Interacting Plant Species", y="Number of Pollen grains (Log)")+theme_bw()
...
ggplot()+
  geom_errorbar(beenat, mapping=aes(x=beenat[,1], ymin=SE.inf, ymax=SE.sup), width=.2, size=0.5, color="black")+ geom_point(data=beenat, mapping=aes(x=beenat[,1], y=EMMean), size=2.5, shape=19, fill=c("black"), color="black")+theme_bw()+theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
```{r NMDS analysis}
Bees <-read.table("1.Data/Bees_PCA_test.csv",header=T, sep=",")
